...
import { isUserAccessTokenPayload } from '~/models/index.js';
 ...
 */
export const RequireUserAccessToken: RequestHandler = (req, res, next) => {
	...
	return Users.readEntryWhereID(user.id)
		.then((data) => {
			if (!data) {
				res.status(403).end();
				return;
