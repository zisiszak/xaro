...
export type InsertCaller<Result> = <DB, TB extends keyof DB, O extends InsertResult>(
...
export type insertOnConflictDoNothing = InsertCaller<Promise<number | null | undefined>>;
/**
 ...
 */
export const insertOnConflictDoNothing: InsertCaller<Promise<number | null | undefined>> = async (
	queryBuilder,
) =>
	queryBuilder
 ...
 * @param queryBuilder - Query Builder instance.
 */
export const insertOnConflictThrow: InsertCaller<Promise<number | undefined>> = async (
	queryBuilder,
) =>
	queryBuilder
		.executeTakeFirstOrThrow()
		.then((result) =>
			typeof result.insertId === 'undefined' ? undefined : Number(result.insertId),
		);
