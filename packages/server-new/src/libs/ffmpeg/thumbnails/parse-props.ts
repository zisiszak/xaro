...
function formatTimestamps(timestamps: readonly Readonly<ThumbnailTimestamp>[]): string[] {
...
function formatSizeString({ width, height }: Readonly<ThumbnailSize>): string {
	const safeWidth = typeof width === 'number' ? Math.round(Math.max(0, width)) : ('?' as const);
	const safeHeight =
		typeof height === 'number' ? Math.round(Math.max(0, height)) : ('?' as const);
...
export function parsePropsFactory(trueBasename: string) {
	return (props: Readonly<ThumbnailConfig>) => {
		const result = {
