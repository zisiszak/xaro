...
export const checkUsernameForValidationErrors = (
	username: string,
): undefined | UsernameValidationError => {
...
export function assertValidUsername(username: string): asserts username is string {
	const length = username.length;
	if (
		length > MAX_USERNAME_LENGTH ||
		length < MIN_USERNAME_LENGTH ||
