import { cleanInt, cleanString } from '@xaro/utils';
import type { ReferenceExpression, SelectQueryBuilder } from 'kysely';
import { type DatabaseSchema } from '../../data/model/database.js';

export type LimitOrderSort<
	TB extends keyof DatabaseSchema,
	OrderBy extends ReferenceExpression<
		DatabaseSchema,
		TB
	> = ReferenceExpression<DatabaseSchema, TB>,
> = {
	limit?: number;
	direction?: 'asc' | 'desc';
	offset?: number;
...
export type ParseLimitOrderSortOptions<TB extends keyof DatabaseSchema> = {
	...
	orderReferenceMapping?: {
		[K in ReferenceExpression<DatabaseSchema, TB> as K extends
			| string
			| number
			| symbol
			? K
...
export const parseLimitOrderSortFactory =
	<TB extends keyof DatabaseSchema>(
		options: ParseLimitOrderSortOptions<TB>,
	) =>
	(queryParams: GenericLimitOrderSortReqQueryParams) => {
		const result: LimitOrderSort<
			TB,
			ReferenceExpression<DatabaseSchema, TB>
		> = {
			offset: cleanInt(queryParams.offset ?? queryParams.OFFSET),
			direction:
				(queryParams.direction ?? queryParams.DIRECTION) === 'asc'
					? 'asc'
					: 'desc',
		};
		...
		if (ord) {
			if (options.orderReferenceMapping) {
				...
				if (mapped) {
					...
					result.orderBy = ord as ReferenceExpression<
						DatabaseSchema,
						TB
					>;
				}
