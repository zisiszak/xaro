...
export const UploadContentFileController: RequestHandler<Params> = async (req, res) => {
		...
		db
			...
			.then((result) => {
				if (!result) {
					...
					return Promise.reject(
						newError({
							message: 'Registered upload item not found in database.',
							log: 'error',
							context: {
								userId,
								clientFileHash,
			...
			.then(async ([_, files]) => {
				if (!files.media || !files.media[0]) {
					...
					return Promise.reject(
						newError({
							message: 'No uploaded files found in media property.',
							log: 'error',
							context: {
								userId,
								clientFileHash,
				...
				const { filepath, newFilename, originalFilename, size, hash } = files.media[0];
				if (hash !== clientFileHash) {
					res.status(400).end();
					return Promise.reject(
						newError({
							message: 'Client file hash does not match uploaded file hash.',
							log: 'error',
							context: {
								uploadId,
								hash,
				...
				if (filepath !== absoluteDestinationFilePath) {
					...
					return Promise.reject(
						newError({
							...
							message: 'Upload filepath does not match destination filepath',
							log: 'error',
							context: {
								uploadId,
								filepath,
